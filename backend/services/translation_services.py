                ...
                try:
                    ...
                    if any(indicator in error_str for indicator in transient_indicators):
                        if attempt < max_retries:
                            ...
                            logger.warning(f"🔄 Translation retry {attempt + 1}/{max_retries} after {wait_time:.1f}s: {e}")
...
class OpenAITranslator(Translator):
    ...
    def translate_batch(
        ...
        logger.info(f"🚀 Starting OpenAI translation: {len(texts)} segments → {target_language}")
        ...
        if config.DEBUG:
            logger.debug(f"📦 Split into {len(batches)} batches, starting from batch {start_batch}")
        else:
        ...
        for batch_idx, batch_texts in enumerate(batches[start_batch:], start_batch):
            ...
            if config.DEBUG:
                logger.debug(f"📊 Batch {batch_id} pre-request: segments={len(batch_texts)}, global_range=#{batch_global_start}-#{batch_global_end}")
            ...
            try:
                ...
                if translations:
                    ...
                    if config.ALLOW_GOOGLE_FALLBACK:
                        logger.warning(f"🔄 Batch {batch_idx} failed with OpenAI after retries, falling back to Google")
                        ...
                        logger.error(f"❌ Batch {batch_idx} failed with OpenAI after retries")
                ...
                if config.ALLOW_GOOGLE_FALLBACK:
                    try:
                        ...
                        logger.info(f"✅ Batch {batch_idx} completed with Google fallback")
                    except Exception as fallback_error:
                        logger.error(f"❌ Google fallback also failed for batch {batch_idx}: {fallback_error}")
        ...
        if len(all_translations) == expected_total:
            logger.info(f"✅ Translation completed: {len(all_translations)}/{expected_total} segments | {total_waited:.1f}s total wait")
        else:
            logger.warning(f"⚠️ Translation completed with mismatch: {len(all_translations)}/{expected_total} segments | {total_waited:.1f}s total wait")
        
        if config.DEBUG:
            unique_translations = len(set(all_translations)) if all_translations else 0
            logger.debug(f"📊 Final summary: expected={expected_total}, received={len(all_translations)}, unique={unique_translations}")
        return all_translations
        ...
        try:
            ...
            if "rate limit" in error_str or "budget exhausted" in error_str:
                ...
                if len(texts) > 1 and self._batch_tokens_estimate(texts) > 0.8 * config.MAX_TOKENS_PER_BATCH:
                    logger.warning(f"🔄 Re-splitting batch {batch_id} ({len(texts)} segments)")
                    ...
                    for i, mini_batch in enumerate(mini_batches):
                        ...
                        try:
                            ...
                            logger.error(f"❌ Mini-batch {mini_id} failed: {mini_error}")
...
Example: [{{"id": 1, "translation": "..."}}, {{"id": 2, "translation": "..."}}]"""
        ...
        if config.DEBUG:
            logger.debug(f"📊 Batch {batch_id}: {input_tokens} input + {estimated_output} estimated = {input_tokens + estimated_output} total tokens")
        ...
        for budget_attempt in range(max_budget_retries):
            if rate_limiter.acquire_budget(input_tokens, estimated_output):
                ...
                logger.warning(f"💰 Budget exhausted for batch {batch_id}, waiting {wait_time}s (attempt {budget_attempt + 1}/{max_budget_retries})...")
                time.sleep(wait_time)
    ...
    def _make_openai_request_with_retries(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if missing_ids:
                    logger.error(f"❌ Batch {batch_id}: Missing IDs: {sorted(missing_ids)}")
                if extra_ids:
                    logger.warning(f"⚠️ Batch {batch_id}: Extra IDs: {sorted(extra_ids)}")
                ...
                if ids_match and valid_count == len(original_texts):
                    ...
                    logger.info(f"✅ Batch {batch_id}: Perfect ID mapping - {valid_count}/{len(original_texts)} translations")
                    ...
                    if missing_ids:
                        ...
                        try:
                            ...
                            logger.info(f"🔄 Retrying {missing_count} missing segments by ID...")
                            ...
                            if recovered_count == missing_count:
                                logger.info(f"✅ Successfully recovered {recovered_count} missing translations by ID")
                            else:
                                logger.warning(f"⚠️ Only recovered {recovered_count}/{missing_count} missing translations")
                ...
                if missing_final:
                    if not config.ALLOW_TRANSLATION_FALLBACK:
                        ...
                        logger.error(f"❌ STRICT MODE: Missing translations for IDs {missing_final}")
                        ...
                        for i in range(len(cleaned_translations)):
                            if cleaned_translations[i] is None:
                                logger.warning(f"⚠️ Using original text for missing ID {i+1}")
                ...
                if attempt < max_retries and retryable:
                    ...
                    logger.warning(f"🔄 Retry {attempt + 1}/{max_retries} for batch {batch_id} after {wait_time}s")
                    continue
                else:
                    # Log failure
                    rate_limiter.log_batch_operation(
    ...
    def _wait_for_retry_with_headers(self, response=None, attempt: int = 1) -> int:
                ...
                if reset_tokens or reset_requests:
                    try:
                        ...
                        if reset_times:
                            ...
                            logger.info(f"⏳ Using rate limit reset header: {wait_seconds}s")
                    except:
                        pass
        ...
        if not wait_seconds:
            ...
            logger.info(f"⏳ Using exponential backoff: {wait_seconds}s (attempt {attempt})")
