...
### שלושה ימי תיקונים — שוחזר מה-git log (27 קומיטים), לא נכתב בזמן אמת

- **yt-dlp:** ה-pin היה ישן עשרה חודשים — עודכן (‎`1a2138c`). ה-player client
  ‎`android_vr` הוסר כי הוא מחזיר ‎403 (‎`8c37b38`). ‎deno הותקן ב-image — ה-JS
  runtime ש-yt-dlp מזהיר שהוא יצטרך (‎`83ae0b7`). קישור שהועתק באמצע פלייליסט
  נפתר כסרטון אחד ולא כ"רדיו" (‎`65f792c`).
- **מסלול "הורד בלבד":** אופציית MP3 (‎`6cf34e1`, ‎`ba79baa`); כרטיס הסיום אומר
  כמה זמן ההורדה לקחה (‎`f470463`); מסך ההתקדמות אומר *מה* הוא מוריד, בעברית
  (‎`266a840`).
- **כלי Windows עצמאי** (‎`standalone-downloader/`): מוריד MP4/MP3 בנפרד
  מהאפליקציה, ‎EXE נבנה ב-CI תחת pwsh, מעדכן את עצמו (‎`7bf5166`, ‎`c850101`,
  ‎`d8b7f02`).
- **קבצים:** שמות עבריים שורדים (‎`d8b7f02`); לכל משימה הפלטים שלה — ‎task id
  בבסיס השם (‎`12a2043`); קובץ תרגום ישן לא מוגש יותר, והממשק מדווח איזה קוד רץ
  (‎`1e802b8`).
- **כנות בשגיאות:** שגיאה אומרת מה נכשל בשפת המשתמש ושומרת מה ששרד (‎`e50d190`);
  דחיות העלאה מקודדות ומתורגמות (‎`f286d67`); הסטטיסטיקה מציגה את המתרגם
  ש**רץ**, לא זה שהתבקש (‎`48555fb`); בחירת Google מנצחת, ותשובת Google שבורה
  לא נשלחת (‎`9a806da`); כרטיס השגיאה נוקב בספק, וחור ה-tail-batch נסגר
  (‎`edfed9f`); משימה מתה מחזירה ‎404 במקום להסתובב לנצח, וכישלון משוחזר שומר את
  הסיפור שלו (‎`c38e04d`, ‎`3bbb57b`); ארבעה באגים משניים (‎`c63d67f`).
- **תצוגה מקדימה מיידית** בסגנון Telegram — מדביקים קישור, רואים את הסרטון
  (‎`7e594e2`, ‎`23f7d33`).
- **עוגן:** ‎`3242f53` — הבעלים סקר ואישר את מצב המערכת המלא; נקודת חזרה מאומתת.
  ‎README עודכן למציאות (‎`dae9337`).

---

## 2026-09-22

### "הסרטון דורש התחברות" — שלוש בעיות נפרדות שנראו כאחת

- **התסמין:** כל קישור יוטיוב בממשק ענה "הסרטון דורש התחברות לאתר"; לפני כן
  ‎127.0.0.1 לא נטען בכלל, והדפדפן הציג שגיאה על ‎subs.sayai.io.
- **בעיה 1 — Docker Desktop תקוע (לא האפליקציה):** ה-engine ענה ‎500
  ‎`context deadline exceeded` לכל קריאה; ה-port-forwarder על ‎:80 האזין בלי
  שום דבר מאחוריו. הפעלה מחדש פתרה. מה שנראה כשגיאה על ‎127.0.0.1 היה דף ישן
  שנשאר בטאב בזמן שהניווט החדש נתקע.
- **בעיה 2 — הדומיין ‎sayai.io פג ב-2.9 (Namecheap):** ‎DNS מצביע לדף חניה
  (‎HTTP בלבד; ‎HTTPS סוגר חיבור). לא תוקן — החלטה של הבעלים אם לחדש. תסריטי
  ‎`test:e2e:prod:*` לא יעבדו עד אז.
- **בעיה 3 — יוטיוב:** ‎yt-dlp מהקונטיינר קיבל "Sign in to confirm you're not a
  bot". השורש: גוגל סימנה את כתובת ה-IPv4 הביתית (‎`curl -4` → ‎302 לדף sorry,
  ‎`curl -6` → ‎200, אותה שנייה). ה-Mac לא הרגיש כי ‎macOS מעדיף IPv6; רשת
  ה-compose הייתה IPv4 בלבד.
  - **תיקון בשני חלקים, שניהם נדרשים:** ‎`enable_ipv6: true` על רשת ה-compose
    (Docker Desktop נותן ULA) **וגם** ‎`gai.conf` ב-image — בלי הקובץ glibc
    עדיין בוחר IPv4 קודם (label של ULA שונה מיעדים גלובליים, RFC 6724 כלל 5).
  - **אומת:** ה-worker מתחבר ליוטיוב ב-‎`2001:4860:…`; ‎1080p מלא; עם IPv6 כבוי
    נופל ל-IPv4 תוך ‎0.03s; ‎Redis / OpenAI / Gemini / nginx→backend תקינים.
  - **לקח:** הניסיון הראשון (רק החלק של compose) נראה כאילו עבד — כי גוגל הסירה
    את הסימון כשעה אחרי המדידה. סיבה ותיקון נמדדים באותו רגע, לא בהפרש שעה.
- **בעיה 4 — נחשפה רק אחרי 3:** ההורדה הצליחה ונפלה ב-‎`Permission denied:
  /app/downloads`. ה-volume ‎`downloads` אותחל מה-image (‎appuser=999, ‎755)
  ואילו compose מריץ כ-‎`501:20`. תוקן: ‎`chmod 1777 /app/downloads` ב-Dockerfile
  (כמו ‎`yt_dlp_cache`) + ‎chown ידני ל-volume הקיים. למה זה עבד בעבר — לא נמצא.
- **סטטוס:** ‎`855dcc2` ב-‎origin/main. הורדה מלאה אומתה דרך כרום ודרך ה-API על
  ה-image החדש. **אם גוגל תסמן יום אחד גם את ה-IPv6** — הצעד הבא הוא cookies;
  לא נבנה מראש.
